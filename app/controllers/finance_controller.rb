...
SELECT s.admission_no AS id_alumno,
 ...
 WHERE     ffcol.due_date < sysdate()
  ...
  def generate_student_debts_report
    current_user = User.find(session[:user_id])
    logger.debug "Current user => #{current_user.inspect}"
    @student = Student.find_by_admission_no(current_user.username)
    logger.debug "Student found => #{@student.inspect}"
    ...
    unless @student
      logger.debug "ERROR: Student not found"
      # Notify student not found
      flash[:notice] = t('app.controllers.finance_controller.generate_student_debts_report.no_results_not_found')
      # Return to page
      render :action => 'request_student_debts_report'
    end
...
SELECT s.admission_no AS id_alumno,
 ...
 WHERE ffcol.due_date < sysdate()
       ...
       AND c.is_deleted = FALSE
       AND ff.transaction_id IS NULL
       AND s.admission_no = '#{@student.admission_no}'
UNION DISTINCT
SELECT s.admission_no AS id_alumno,
       CONCAT_WS(' ', s.first_name, s.middle_name, s.last_name) AS name, 
       CONCAT_WS(' ', c.course_name, b.name) AS grupo,
 ...
 WHERE     ffcol.due_date < sysdate()
    ...
    if results.empty?
      ...
      flash[:notice] = t('app.controllers.finance_controller.generate_student_debts_report.no_results_not_found')
      # Return to page
      redirect_to :action => 'request_student_debts_report'
    else
      # Render CSV
      respond_to do |format|
        format.csv  { 
