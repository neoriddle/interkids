class ApplicationController < ActionController::Base
  ...
  def permission_denied
    flash[:notice] = t('app.controllers.application_controller.sorry_you_are_not_allowed_to_access_that_page')
  ...
  def protect_other_student_data
    if current_user.student?
      ...
      unless params[:id].to_i == student.id or params[:student].to_i == student.id
        flash[:notice] = t('app.controllers.application_controller.you_are_not_allowed_to_view_that_information')
        redirect_to :controller=>"user", :action=>"dashboard"
      end
    end
  end
  ...
  def protect_leave_dashboard
    ...
    unless employee_user.id == current_user.id
      flash[:notice] = t('app.controllers.application_controller.access_denied')
      redirect_to :controller=>"user", :action=>"dashboard"
    end
  end
