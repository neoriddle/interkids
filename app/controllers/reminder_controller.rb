class ReminderController < ApplicationController
  ...
  def create_reminder
    ...
    if request.post?
      unless params[:reminder][:body] == "" or params[:recipients] == ""
        ...
        flash[:notice] = t('app.controllers.reminder_controller.message_sent_successfully')
        redirect_to :controller=>"reminder", :action=>"create_reminder"
      else
        flash[:notice]= "<b>" + t('app.controllers.reminder_controller.error') + "</b>" + t('app.controllers.reminder_controller.please_fill_the_required_fields_to_create_this_message')
        redirect_to :controller=>"reminder", :action=>"create_reminder"
      end
    end
  end
  ...
  def delete_reminder_by_sender
    ...
    flash[:notice] = t('app.controllers.reminder_controller.reminder_deleted')
    ...
    if request.post?
      unless params[:reminder][:body] == "" or params[:recipients] == ""
        Reminder.create(:sender=>user.id, :recipient=>@sender.id, :subject=>params[:reminder][:subject],
          :body=>params[:reminder][:body], :is_read=>false, :is_deleted_by_sender=>false,:is_deleted_by_recipient=>false)
        flash[:notice]= t('app.controllers.reminder_controller.your_reply_has_been_sent')
        redirect_to :controller=>"reminder", :action=>"view_reminder", :id2=>params[:id2]
      else
        flash[:notice]= "<b>" + t('app.controllers.reminder_controller.error') + "</b>" + t('app.controllers.reminder_controller.please_enter_both_subject_and_body')
        redirect_to :controller=>"reminder", :action=>"view_reminder",:id2=>params[:id2]
      end
    end
  end
  ...
  def mark_unread
    ...
    flash[:notice] = t('app.controllers.reminder_controller.reminder_marked_unread')
  ...
  def send_reminder
    unless params[:create_reminder][:message] == "" or params[:create_reminder][:to] == ""
      ...
      render(:update) do |page|
        page.replace_html 'error-msg', :text=> '<p class="flash-msg">' + t('app.controllers.reminder_controller.your_message_has_been_sent') + '</p>'
      end
    else
      render(:update) do |page|
        page.replace_html 'error-msg', :text=> '<p class="flash-msg">' + t('app.controllers.reminder_controller.please_enter_message_and_subject') + '</p>'
      end
    end
  end
end
